...
class OuvidoriaController extends Controller
{
    ...
    public function alterarOV($id, $id_ouvidoria, Request $request)
    {
        ...
        $unidadesMenu = Unidade::where('status_unidades',1)->get();
		$unidades 	  = $unidadesMenu;
        $ouvidoria 	  = Ouvidoria::where('status_ouvidoria', 1)->find($id_ouvidoria);
		if($validacao == 'ok') {
    ...
    public function excluirOV($id, $id_ouvidoria, Request $request)
    {
        ...
        $ouvidoria    = Ouvidoria::find($id_ouvidoria);
		if($validacao == 'ok') {
    ...
    public function storeOV($id, Request $request)
    {
		...
		if ($validator->fails()) {
			...
			$ouvidorias  = Ouvidoria::where('status_ouvidoria',1);
	...
	public function storeOVRelatorioES($id, Request $request)
	{
		...
		if ($validacao == 'ok') {
			...
			if ($validator->fails()) {
				...
				$Relatorio = OuvidoriaRelEstatis::where('mes', $input['mes'])->where('ano', $input['ano'])->where('status_ouvi_rel_estas', 1)->where('unidade_id',$id)->get();
				if (sizeof($Relatorio) > 0) {
					...
					if ($extensao !== 'pdf') {
						...
						$siglaUnd = (Unidade::where('id', $id)->get())[0]->sigla;
	...
	public function statusOVRelatorioES($id, $id_doc, Request $request)
	{
		...
		if ($validacao == 'ok') {
			...
			if ($relEstatisc->status_ouvi_rel_estas == 0) {
				$relatorio = OuvidoriaRelEstatis::where('id', '<>', $id_doc)->where('mes', $relEstatisc->mes)->where('ano', $relEstatisc->ano)->where('status_ouvi_rel_estas', 1)->where('unidade_id',$id)->get();
				if (sizeof($relatorio) > 0) {
	...
	public function telaInativarOVRelatorioES($id, $id_doc, Request $request)
	{
		$validacao = permissaoUsersController::Permissao($id);
		$unidade      = Unidade::find($id);
		$unidadesMenu = Unidade::where('status_unidades', 1)->get();
		$unidades 	  = $unidadesMenu;
		if ($validacao == 'ok') {
			$relatoriosEs = OuvidoriaRelEstatis::where('id', $id_doc)->get();
			return view('transparencia/ouvidoria/ouvidoria_rel_estatis_inativar', compact('unidades', 'unidadesMenu', 'unidade', 'relatoriosEs'));
		} else {
			$validator = 'Você não tem Permissão!!';
			return view('home', compact('unidades', 'unidade', 'unidadesMenu'))
				->withErrors($validator)
	...
	public function inativarOVRelatorioES($id, $id_doc, Request $request)
	{
		$input = $request->all();
		$relEstatisc = OuvidoriaRelEstatis::find($id_doc);
		if($relEstatisc->status_ouvi_rel_estas == 1) {
			$nomeArq    = explode($relEstatisc->name_arq, $relEstatisc->file_path); 
			$nome       = "old_". $relEstatisc->name_arq; 
			$image_path = $nomeArq[0].$nome;  
			DB::statement("UPDATE ouvidoria_rel_estatis SET `status_ouvi_rel_estas` = 0, `file_path` = '$image_path', `name_arq` = '$nome' WHERE `id` = $id_doc");
			$caminho = $relEstatisc->file_path;
			rename($caminho, $image_path);
		} else {
			$nomeArq    = explode($relEstatisc->name_arq, $relEstatisc->file_path);
            $nome       = explode("old_", $relEstatisc->name_arq); 
		...
		$input['registro_id'] = $relEstatisc->id;
		$log          = LoggerUsers::create($input);
		$lastUpdated  = $log->max('updated_at');
        $unidadesMenu = Unidade::where('status_unidades',1)->get();
		$unidades 	  = $unidadesMenu;
		$unidade      = Unidade::where('status_unidades',1)->find($id_escolha);
		$regimento    = OuvidoriaRelEstatis::where('unidade_id',$id_escolha)->get();
		$validator    = 'Relatório Estatístico inativado com sucesso!';
		return redirect()->route('cadastroOVRelatorioES', [$id_escolha])
				->withErrors($validator);
	}
}