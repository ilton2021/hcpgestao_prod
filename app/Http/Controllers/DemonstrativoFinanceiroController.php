...
class DemonstrativoFinanceiroController extends Controller
{
    ...
    public function storeDF($id_unidade, Request $request)
    {
        ...
        $mes = $input['mes'];
        $ano = $input['ano'];
        $financialR = DemonstrativoFinanceiro::where('unidade_id',$id_unidade)->where('mes',$mes)->where('ano',$ano)->where('tipodoc', $input['tipodoc'])->where('tipoarq',$input['tipoarq'])->where('periodo',$input['periodo'])->get();
                        ...
                        if ($validator->fails()) {
                            ...
                            for ($i = 1; $i <= $qtdUnidades; $i++) {
                                if ($unidade['id'] === $i) {
                                    ...
                                    } else if($input['title'] == 3){
                                        $title = '.relat-anual-finan-';
                                        $input['title'] = "Relátorio Anual";
                                    }else {
                                    ...
                                    if ($input['tipodoc'] == 2) {
                                        ...
                                        $tipodoc = "Prefeitura-";
                                    }elseif ($input['tipodoc'] == 5) {
                                        $input['title'] = $input['title'] . '- Emenda';
                                        $tipodoc = "Emenda-";
                                    } else {
                                        $tipodoc = "";
                                    }
