			 ...
			 @foreach ($permissao_users as $permissao)
			  @if(($permissao->permissao_id == 8) && ($permissao->user_id == Auth::user()->id))
			   @if ($permissao->unidade_id == $unidade->id)
				...
				@if($unidade->id == 8)
				 <p align="right"><a href="{{route('cadastroRAC', $unidade->id)}}" class="btn btn-info btn-sm" style="color: #FFFFFF;"> <b>Alterar</b> <i class="fas fa-edit"></i> </a></p>
				@else
				 <p align="right"><a href="{{route('cadastroRA', $unidade->id)}}" class="btn btn-info btn-sm" style="color: #FFFFFF;"> <b>Alterar</b> <i class="fas fa-edit"></i> </a></p>
			    </div>
			    @endif
			   @endif
			  @endif
		...
		<div class="col-md-10 col-sm-12 text-center">
			@foreach($anosRef as $year)
			  <div class="col-2 d-inline-flex flex-wrap">
				<div class="p-2">
					<button class="btn btn-success btn-md" type="button" data-toggle="collapse" data-target="#{{$year}}" aria-expanded="false" aria-controls="{{$year}}">
						{{$year}}
					</button>
				</div>
			  </div>
			...
			<div class="collapse border-0" id="{{$year}}">
				<div class="card border-0 card-body" style="background-color: #fafafa;">
					...
					<div class="d-inline-flex flex-wrap justify-content-center text-center">
						...
						@else
						<a href="{{route('visualizarAssistencial', ['id' => $unidade->id, 'year' => $year])}}" class="btn btn-info btn-sm" style="color: #FFFFFF;"> <b>Visualizar</b> <i class="fas fa-bars"></i> </a>
						@endif
					</div>
				</div>
			</div>
