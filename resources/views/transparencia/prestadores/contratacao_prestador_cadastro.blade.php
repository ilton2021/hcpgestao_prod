					...
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<table class="table table-sm">
							<tr>
							  <td> Prestador: </td>
							  <td>  
								<input style="width: 500px" class="form-control form-control-sm" type="text" id="prestador" name="prestador" value="{{ old('prestador') }}" required />
							  </td>
						    </tr>
							<tr>
							  <td> CNPJ/CPF: </td>
							  <td> <input style="width: 200px" class="form-control form-control-sm" type="text" id="cnpj_cpf" name="cnpj_cpf" value="{{ old('cnpj_cpf') }}" required /> </td>
							</tr>
							<tr>
							  <td> Tipo Contrato: </td>
							  <td> 
								<select name="tipo_contrato" id="tipo_contrato" style="width: 300px;" class="form-control form-control-sm">	
									<option value="OBRAS" <?php if(old('tipo_contrato') == 'OBRAS') { echo 'selected'; } ?>> OBRAS </option>
									<option value="SERVIÇOS" <?php if(old('tipo_contrato') == 'SERVIÇOS') { echo 'selected'; } ?>> SERVIÇOS </option>
									<option value="AQUISIÇÃO" <?php if(old('tipo_contrato') == 'AQUISIÇÃO') { echo 'selected'; } ?>> AQUISIÇÃO </option>
								</select>
							  </td>
							</tr>
							<tr>
							  ...
							  <td> 
								<select name="tipo_pessoa" id="tipo_pessoa" style="width: 300px;" class="form-control form-control-sm">	
									<option value="PESSOA FÍSICA" <?php if(old('tipo_pessoa') == 'PESSOA FÍSICA') { echo 'selected'; } ?>> PESSOA FÍSICA </option>
									<option value="PESSOA JURÍDICA" <?php if(old('tipo_pessoa') == 'PESSOA JURÍDICA') { echo 'selected'; } ?>> PESSOA JURÍDICA </option>
								</select>
							  </td>
							</tr>
						</table>	
