...
<div class="container-fluid">
    <div class="row" style="margin-top: 25px;">
        <div class="col-md-12 text-center">
            ...
            <div class="p-2">
                <a href="{{route('transparenciaAssistencial', array($unidade->id,1))}}" class="btn btn-warning btn-sm" style="color: #FFFFFF;"> <i class="fas fa-reply"></i> <b>Voltar</b> </a>
            </div>
        </div>
    </div>
    @if ($errors->any())
    ...
    <div class="row mt-2 form-control">
        <div class="d-flex d-flex-inline justify-content-center">
            ...
            <div class="p-2">
                <a class="btn btn-info btn-sm" style="color: #FFFFFF;" href="{{route('cadastroRADOC', $unidade->id)}}"> <b>Alterar</b> <i class="fas fa-edit"></i> </a></li>
            </div>
        </div>
                                    ...
                                    <div class="d-flex flex-column" id="headingOne">
                                        <div class="d-md-inline-flex justify-content-between border-bottom border-success align-items-center">
                                            ...
                                            <div class="p-1 text-center">
                                                <a href="{{asset($AD->file_path)}}" target="_blank" class="btn btn-success btn-sm"><b>Download</b> <i class="bi bi-download"></i></a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @foreach($assistencials as $AD)
                         @if($AD->ano_ref == $year)
                          @if($AD->status_assistencials == 1)
                           <a title="Inativar" class="btn btn-warning btn-sm" href="{{route('telaInativarRA', array($unidade->id, $year, 0))}}"> <i class="fas fa-times-circle"></i></a>
                           @break
                          @else
                           <a title="Ativar" class="btn btn-success btn-sm" href="{{route('telaInativarRA', array($unidade->id, $year, 1))}}"> <i class="fas fa-times-circle"></i></a>
                           @break
                          @endif
                         @endif
                        @endforeach
